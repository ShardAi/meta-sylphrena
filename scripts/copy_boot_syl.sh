...
if [ -z "$OETMP" ]; then
    ...
    SRCDIR=~/Workspace/RaspberryPi/poky/build/tmp/deploy/images/raspberrypi3/
else
    echo -e "\nOETMP: $OETMP"
...
for f in ${BOOTLDRFILES}; do
    if [ ! -f ~/Workspace/RaspberryPi/poky/build/tmp/deploy/images/raspberrypi3/bcm2835-bootfiles/${f} ]; then
...
have_one_dtb=0
for f in ${DTBS}; do
    if [ -f ~/Workspace/RaspberryPi/poky/build/tmp/deploy/images/raspberrypi3/${KERNEL_IMAGETYPE}-${f} ]; then
        have_one_dtb=1
    fi
done
...
if [ ! -f ~/Workspace/RaspberryPi/poky/build/tmp/deploy/images/raspberrypi3/${KERNEL_IMAGETYPE} ]; then
...
for f in ~/Workspace/RaspberryPi/poky/build/tmp/deploy/images/raspberrypi3/${KERNEL_IMAGETYPE}-*.dtbo; do
    if [ -L $f ]; then
